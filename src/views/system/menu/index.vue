<template>
  <div>
    <el-card class="container-card" shadow="always">
      ...
      <el-table v-loading="loading" :tree-props="{children: 'children', hasChildren: 'hasChildren'}" row-key="menuId" :data="tableData" border stripe style="width: 100%" @selection-change="handleSelectionChange">
        <el-table-column type="selection" width="55" align="center" />
        <el-table-column  prop="title" label="菜单标题" width="150" align="center" />
        ...
        <el-table-column  prop="status" label="禁用" align="center" width="80">
          <template slot-scope="scope">
            <el-tag size="small" :type="scope.row.status === 2 ? 'success':'danger'">{{ scope.row.status === 2 ? '否':'是' }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column show-overflow-tooltip prop="hidden" label="隐藏" align="center" width="80">
          <template slot-scope="scope">
            <el-tag size="small" :type="scope.row.hidden === 2 ? 'danger':'success'">{{ scope.row.hidden === 2 ? '是':'否' }}</el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column align="center" prop="activeMenu" label="高亮菜单" />
        <el-table-column fixed="right" label="操作" align="center" width="120">
          <template slot-scope="scope">
            ...
            <el-tooltip class="delete-popover" content="删除" effect="dark" placement="top">
              <el-popconfirm title="确定删除吗？" @onConfirm="singleDelete(scope.row.menuId)">
...
import { getMenuTree, createMenu, updateMenu, batchDeleteMenu } from '@/api/system/menu'
  ...
  methods: {
    ...
    async getTableData() {
      ...
      try {
        ...
        this.tableData = data
        this.treeselectData = [{ menuId: 0, title: '顶级类目', children: data }]
      } finally {
        this.loading = false
      }
    },
    ...
    // 修改
    update(row) {
      this.dialogFormData.menuId = row.menuId
      ...
      this.dialogFormData.status = row.status === 2 ? '否' : '是'
      this.dialogFormData.hidden = row.hidden === 2 ?  '是' : '否' 
      this.dialogFormData.noCache = row.noCache === 2 ? '是' : '否'
      this.dialogFormData.activeMenu = row.activeMenu
      this.dialogFormData.alwaysShow = row.alwaysShow
      this.dialogFormData.breadcrumb = row.breadcrumb 
      if (row.pid) {
    ...
    submitForm() {
      this.$refs['dialogForm'].validate(async valid => {
        if (valid) {
          ...
          if (this.dialogFormData.meunId === this.dialogFormData.pid) {
            return this.$message({
              showClose: true,
          ...
          this.dialogFormData.status = this.dialogFormData.status === '否' ? 2 : 1
          this.dialogFormData.hidden = this.dialogFormData.hidden === '是' ? 2 : 1
          this.dialogFormData.noCache = this.dialogFormData.noCache === '是' ? 2 : 1
          console.log("----------------------", this.dialogFormData)
          const dialogFormDataCopy = { ...this.dialogFormData, pid: this.treeselectValue }
          ...
          try {
            if (this.dialogType === 'create') {
              ...
              const { message } = await updateMenu(dialogFormDataCopy)
              msg = message
            }
          } finally {
    ...
    batchDelete() {
      this.$confirm('此操作将永久删除, 是否继续?', '提示', {
        ...
        this.multipleSelection.forEach(x => {
          menuIds.push(x.menuId)
        })
        let msg = ''
        try {
          const { message } = await batchDeleteMenu({ menuIds: menuIds })
          msg = message
        } finally {
          this.loading = false
        }
    ...
    async singleDelete(Id) {
      ...
      let msg = ''
      try {
        const { message } = await batchDeleteMenu({ menuIds: [Id] })
        msg = message
      } finally {
        this.loading = false
      }
